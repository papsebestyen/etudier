...
def main():
    ...
    parser.add_argument(
        "url", help="URL for a Google Scholar search to start collecting from"
    )
    parser.add_argument(
        "--depth",
        type=int,
        default=1,
        help="depth of the crawl in terms of levels of citation (defaults to 1)",
    )
    parser.add_argument(
        "--pages",
        type=int,
        default=1,
        help="breadth of the crawl in terms of number of pages of results (defaults to 1)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="output",
        help="file prefix to use for the output files (defaults to 'output')",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        default=False,
        help="display diagnostics during the crawl",
    )
...
def get_html(url):
    ...
    while True:
        try:
            ...
            try:
                html = driver.find_element(By.CSS_SELECTOR, "#gs_top").get_attribute(
                    "innerHTML"
                )
...
def write_html(g, depth, output):
    ...
    html = tmpl.substitute({"__OPTIONS__": opts, "__GRAPH_JSON__": graph_json})
